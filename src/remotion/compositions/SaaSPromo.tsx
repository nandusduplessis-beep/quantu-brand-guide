...
import { BRAND, BRAND_THEMES, MONOGRAM_PATHS, type BrandThemeKey, hexToRgba } from '../brand';
...
// ─── Animated Hexagon Monogram ────────────────────────────────────────────────

const HexMonogram: React.FC<{
  color: string;
  size: number;
  frame: number;
  fps: number;
}> = ({ color, size, frame, fps }) => {
  // Breathing animation driven by frame (not CSS)
  const breathCycle = (frame / fps) * Math.PI * 0.5; // 4s full cycle
  const outerOpacity = 0.3 + 0.15 * Math.sin(breathCycle);
  const middleOpacity = 0.7 + 0.15 * Math.sin(breathCycle + 0.4);
  const innerOpacity = 1.0 - 0.12 * Math.sin(breathCycle + 0.8);

  const scale = size / 367;
  ...
  return (
    <svg
      ...
      style={{ width: size, height: size * (418 / 367) }}
    >
      <g opacity={outerOpacity} transform="translate(0, 2.81)">
        <path d={MONOGRAM_PATHS.outer} fill={color} fillRule="nonzero" stroke={color} strokeWidth={12.628 * scale} />
      </g>
      <g opacity={middleOpacity} transform="translate(38.19, 46.65)">
        <path d={MONOGRAM_PATHS.middle} fill={color} fillRule="nonzero" stroke={color} strokeWidth={12.628 * scale} />
      </g>
      <g opacity={innerOpacity} transform="translate(78.95, 93.04)">
        <path d={MONOGRAM_PATHS.inner} fill={color} fillRule="nonzero" stroke={color} strokeWidth={12.628 * scale} />
      </g>
    </svg>
  );
};
...
const HeroScene: React.FC<{
  ...
  return (
    ...
    >
      ...
      >
        <HexMonogram color={theme.accent} size={400} frame={frame} fps={fps} />
      </div>
...
const CTAScene: React.FC<{
  ...
  return (
    ...
    >
      ...
      >
        <HexMonogram color={theme.accent} size={500} frame={frame} fps={fps} />
      </div>
